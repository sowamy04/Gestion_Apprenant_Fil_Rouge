...
class ApiController extends AbstractController
{
  ...
  /**
   * @Route("/api/updateProfil/{id}", name="update_profil", methods={"POST"})
   */
  // public function modifierProfil(Request $request, SerializerInterface $serializer, EntityManagerInterface $em)
  // {
  //   $jsonRecu = $request->getContent();
  ...
  public function modifierProfil(Profil $profil, Request $request, ManagerRegistry $manager)
  {
    ...
    if ($profil->getId()) {
      dd($profil);
      $managers = $manager->getManager();
      $managers->persist($profil);
      $managers->flush();
      return $this->json([
        'code' => 200,
        'message' => 'modifier avec avec succé'
      ]);
    } else {
      return $this->json([
